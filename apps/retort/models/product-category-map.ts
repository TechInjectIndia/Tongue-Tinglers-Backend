...
export class RetortProductCategoryMapRepo implements IBaseRepo<TRetortProductCategoryLink, TRetortProductFilters> {
    ...
    public async deleteByProductId(productId: number): Promise<void> {
        await RetortProductCategoryMapModel.destroy({
            where: { productId },
        });
    }

    public async bulkCreate(categoryMappings: TAddRetortProductCategoryLink[]): Promise<void> {
        await RetortProductCategoryMapModel.bulkCreate(categoryMappings);
    }
