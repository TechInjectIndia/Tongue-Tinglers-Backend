...
const createCommissionSchema = Joi.object({
    ...
    type: Joi.string().valid("absolute", "percentage").required().messages({
    ...
    eventType: Joi.string()
        .valid(
            "master-franchise-franchise-sold",
            "master-franchise-raw-material-sold",
            "affiliate-franchise-sold",
            "affiliate-raw-material-sold",
        )
        .required()
        .messages({
...
const validateUpdateCommission = (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
        ...
        return res
            ...
            .send(
                HelperMethods.getErrorResponse(
                    "Id of the commission to be updated is required.",
                ),
            );
    }
    ...
    if (error) {
        return res
            .status(400)
            .json(HelperMethods.getErrorResponse(error.details[0].message));
    }
    next();
};
...
const validateDeleteCommission = (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
...
const validateGetCommissionById = (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
    ...
    if (!id || id <= 0) {
        return res
            .status(400)
            .send(HelperMethods.getErrorResponse("Invalid request"));
    }
    next();
};
...
const validateCreateCommissionMapEntry = (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
...
const validateSearchCommission = (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
    ...
    if (!title || title.length === 0) {
        return res
            .status(400)
            .json(HelperMethods.getErrorResponse("Invalid request"));
    }
    next();
};
...
const validateUpdateCommissionEntityStatus = (
    req: Request,
    res: Response,
    next: NextFunction,
) => {
