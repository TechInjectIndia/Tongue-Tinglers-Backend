...
export class ZohoSignRepo implements IBaseRepo<TemplateType> {
        ...
        return await this.handleTokenError(async () => {
            try {
                ...
                const response = await axios.get(
                    `${ZOHO_API_URL}/requests/${documentId}`,
                    {
                        headers: {
                            Authorization: `Zoho-oauthtoken ${accessToken}`,
                        },
    ...
    public async getTemplateFields(templateId: string): Promise<FieldType> {
        return await this.handleTokenError(async () => {
            try {
                const accessToken =
                    await new ZohoSignRepo().getAccessTokenFromDb();
                const response = await axios.get(
                    `${ZOHO_API_URL}/templates/${templateId}`,
                    {
                        headers: {
                            Authorization: `Zoho-oauthtoken ${accessToken}`,
                        },
                ...
                return {
                    docs:
                        response.data.templates.document_fields.map(
                            (docs: any) => ({
