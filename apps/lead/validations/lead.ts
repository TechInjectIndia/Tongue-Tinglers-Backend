...
const FOLLOWED_DATE_SCHEMA = Joi.object().keys({
  followedDate: Joi.date().iso().allow(null).messages({
    "date.iso": "Followed date must be in ISO format.",
  }),
  followedBy: USER_DETAILS_SCHEMA.required().messages({
    "any.required": "Followed By details are required.",
  }),
  notes: Joi.string().allow(null).optional(),
  description: Joi.string().allow(null).optional(),
  status: Joi.string()
    .valid(...Object.values(followStatus))
    .messages({
      "any.only": `Status must be one of: ${Object.values(followStatus).join(
        ", "
      )}.`,
    }),
  createdAt: Joi.date().iso().allow(null).messages({
    "date.iso": "Followed date must be in ISO format.",
  }),
  createdBy: USER_DETAILS_SCHEMA.required().messages({
    "any.required": "Followed By details are required.",
  }),
  reminder: Joi.date().iso().allow(null).messages({
...
const ASSIGN_SCHEMA = Joi.object().keys({
  assignedTo: USER_DETAILS_SCHEMA.required().messages({
  ...
  affiliate: Joi.array()
    .items(
      Joi.object().keys({
        id: Joi.number()
          .required()
          .messages({ "any.required": "Affiliate ID is required." }),
        name: Joi.string()
          .required()
          .messages({ "any.required": "Affiliate name is required." }),
      })
    )
    .optional()
    .allow(null),
