...
import { InvoicePDFProvider } from "apps/invoice/InvoiceProvider/InvoicePDFProvider";
import {
    OrderStatus,
    ParsedOrder,
    PAYMENT_TYPE,
...
server.get("/harsh", async (_, res) => {
    ...
    const resp = await obj.generate(order);
    res.send(resp);
});

server.get("/invoice", async (_, res) => {
    const email = "harshdalal.techinject@gmail.com"
    const mailDto = await new LeadToProspectMail().getPayload(
        {},
        email,
    );
    // console.log(mailDto);
    // const rr= await sendMail(mailDto);
    res.send(mailDto)
});
