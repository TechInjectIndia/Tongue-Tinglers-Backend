...
const SMDetailsSchema = Joi.object({
    handle: Joi.string().required().messages({
        "string.base": "SM handle must be a string.",
        "any.required": "SM handle is required.",
    }),
    followers: Joi.number().integer().required().messages({
...
const editAffiliateBody = Joi.object({
    ...
    codes: Joi.object()
        ...
        .messages({
            "object.base":
                "Codes must be an object with string key-value pairs.",
        }),
    sm: Joi.object()
        .pattern(Joi.string(), SMDetailsSchema)
        .optional()
...
const listAffiliateQuery = Joi.object({
    ...
    sorting: Joi.string().trim().allow("").optional().messages({
