  ...
  static async list(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      return res
        .status(200)
        .send(
          sendResponse(RESPONSE_TYPE.SUCCESS, SUCCESS_MESSAGE.FETCHED, Products)
        );
    } catch (err) {
      console.log(err);
      return res.status(500).send({
  ...
  static async delete(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (!Array.isArray(ids) || ids.length === 0) {
        return res
          .status(400)
          .send(sendResponse(RESPONSE_TYPE.ERROR, "Invalid ids provided"));
      }
      ...
      if (!deleted) {
        return res
          .status(404)
          .send(sendResponse(RESPONSE_TYPE.ERROR, "Contracts not found"));
      }
      return res
        .status(200)
        .send(sendResponse(RESPONSE_TYPE.SUCCESS, SUCCESS_MESSAGE.DELETED));
    } catch (error) {
      return res.status(500).send({
        message: ERROR_MESSAGE.INTERNAL_SERVER_ERROR,
      });
    ...
    try {
      ...
      const paymentId =
        existingContract.payment && existingContract.payment.length > 0
          ? existingContract.payment[existingContract.payment.length - 1]
              .paymentId
          : null;
      const signId =
        existingContract.signedDocs && existingContract.signedDocs.length > 0
          ? existingContract.signedDocs[existingContract.signedDocs.length - 1]
              .docId
          : null;
