...
const invoiceHandler = async (
    ...
    try {
        ...
        if (!body || typeof body !== "object") {
            response
                .status(400)
                .send(getSuccessDTO({ error: "No payload found" }));
            return;
        }
        ...
        if (!type || (type !== "invoice" && type !== "shippingNote")) {
            response
                .status(400)
                .send(getSuccessDTO({ error: "Type not found" }));
            return;
        }
        ...
        if (!orderId || isEmpty(orderId)) {
            response
                .status(400)
                .send(getSuccessDTO({ error: "Order ID not found" }));
            return;
        }

        const orderDto =
            await ServerRepoProvider.orderRepo.getOrderById(orderId);

        if (!orderDto.success) {
            response
                .status(404)
                .send(getSuccessDTO({ error: "Order not found" }));
            return;
        }
        ...
        response
            .status(500)
            .send(getSuccessDTO({ error: "Internal Server Error" }));
    }
};
