...
export default class ProductController {
    ...
    static async getAllProducts(req: Request, res: Response) {
        try {
            ...
            const products: Pagination<Product> = await RepoProvider.ProductRepo.getAll(page, limit, search, filterObj);
            return res.status(200)
                    .send(
                        sendResponse(
                            RESPONSE_TYPE.SUCCESS,
    ...
    static async updateStatus(req: Request, res: Response) {
        try {
            ...
            payload.id = id;
            const product: Product = await RepoProvider.ProductRepo.changeStatus(payload);
            return res.status(200)
                    .send(
                        sendResponse(
                            RESPONSE_TYPE.SUCCESS,
