...
export class FranchiseRepo implements IFranchiseRepo {
    async create(franchise: FranchiseDetails, userId: number,
        options?: { transaction?: any }): Promise<Franchise | null> {
        try {
            ...
            if (!existFranchise) {
                ...
                if (franchise.sm.length > 0) {
                    const smDetails = await RepoProvider.smRepo.saveBulk(
                        franchise.sm);
    ...
    async update(franchise: Franchise): Promise<Franchise> {
        ...
        //   const existingFranchise = await
        // FranchiseModel.findByPk(franchiseId); if (!existingFranchise) {
        ...
        //     addressId = (
        //       await RepoProvider.address.updateById(franchise.location,
        // addressId) ).id; }

        //   // Update SM (if provided)
        //   let smIds: number[] = existingFranchise.sm || [];
        //   if (franchise.sm && franchise.sm.length > 0) {
        //     const smDetails = await
        // RepoProvider.smRepo.saveBulk(franchise.sm); smIds =
        // smDetails.map((sm) => sm.id); }
        ...
        //     pocPhoneNumber: franchise.pocPhoneNumber ||
        // existingFranchise.pocPhoneNumber, users: franchise.users ||
        // existingFranchise.users, regionId: franchise.regionId ||
        // existingFranchise.regionId, area: franchise.area ||
        // existingFranchise.area, agreementIds: franchise.agreementIds ||
        // existingFranchise.agreementIds, paymentIds: franchise.paymentIds ||
        // existingFranchise.paymentIds, status: franchise.status ||
        // existingFranchise.status, establishedDate: franchise.establishedDate
        // || existingFranchise.establishedDate, organizationId:
        // franchise.organizationId || existingFranchise.organizationId,
        // location: addressId, sm: smIds, updatedBy: getUserName(user), });

        //   await existingFranchise.save();

        //   console.log(`Franchise with ID ${franchiseId} updated
        // successfully.`); return existingFranchise.toJSON(); } catch (e) {
        // console.error(`Error updating franchise with ID ${franchiseId}:`,
        // e); return null; }
        ...
        try {
            ...
            const {rows: franchise, count: total} =
                await FranchiseModel.findAndCountAll({
                    ...
                    return {
                        rows: res.rows.map(
                            (product) => parseFranchise(product.toJSON())),
                        count: res.count,
                    };
                });
